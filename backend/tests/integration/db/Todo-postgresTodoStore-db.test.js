...
describe('TodoModel + PostgresTodoStore (Integration test)', () => {
  ...
  describe('update', () => {
    ...
    test('You can update the title field in a TODO in the DB.', async () => {
      ...
      const updated = await model.update(existenId, { title: 'Edited Title' });
      expect(updated.id).toEqual(existenId);
      expect(updated.title).toEqual('Edited Title');
      ...
      expect(rows[0].completed).toEqual(false);
    });

    test('You can update the archived field in a TODO in the DB.', async () => {
      ...
      const updated = await model.update(existenId, { archived: true });
      expect(updated.id).toEqual(existenId);
      expect(updated.title).toEqual('ToDo 1');
      expect(updated.completed).toEqual(false);
      expect(updated).not.toHaveProperty('archived');
      ...
      expect(rows[0].title).toBe('ToDo 1');
      expect(rows[0].completed).toEqual(false);
      expect(rows[0].archived).toEqual(true);
    });
